...
if(isset($_REQUEST['start'])) {
	if($_SESSION['login']['user']=="guest") {
		...
		if(file_exists("$base_dir$server_select/server-settings.json")) {
			...
			$output = shell_exec( __DIR__ . '/../factorio/manage.new.sh "'.$server_select.'" "prestart" "'.$_SESSION['login']['user'].'"');
			...
			$output = shell_exec( __DIR__ . '/../factorio/manage.new.sh "'.$server_select.'" "start" "'.$_SESSION['login']['user'].'"');
			echo $output;
		} else {
	...
	$output = shell_exec( __DIR__ . '/../factorio/manage.new.sh "'.$server_select.'" "status" "'.$_SESSION['login']['user'].'"');
	echo $output;
	...
	if($_SESSION['login']['user']=="guest") {
		...
		$output = shell_exec( __DIR__ . '/../factorio/manage.new.sh "'.$server_select.'" "stop" "'.$_SESSION['login']['user'].'"');
		echo $output;
	}
